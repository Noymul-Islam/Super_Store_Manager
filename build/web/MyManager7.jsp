    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            try {
                Statement st = conn.createStatement();
                 ResultSet rs = null;
                if (spo.tn.equals("monthdata")){
                    rs = st.executeQuery("select month,total_sold from "+spo.tn);
                    while (rs.next()) {
                        double d = rs.getDouble("total_sold");
                        t_sold[counter] = Double.toString(d);
                        nameOfItems[counter] = rs.getString("month");
                        ++counter;
                    }
                }   
                else if (spo.tn.equals("yeardata")){
                    rs = st.executeQuery("select year,total_sold from "+spo.tn);
                    while (rs.next()) {
                        double d = rs.getDouble("total_sold");
                        t_sold[counter] = Double.toString(d);
                        nameOfItems[counter] = rs.getString("year");
                        ++counter;
                    }
                }   
            }
            ...
            <%}%>
                ...
                // Set chart options
                var options = {
                    'title': '<%=spo.tn%> Progress!!!', //title which will be shown right above the Google Pie Chart
